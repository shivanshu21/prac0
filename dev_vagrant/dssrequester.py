...
def initiate(printer, action, target=None, file=None):
    ...
    try:
        if COMMON_DSS_URL is None:
            print "Global variable dss_url not set!"
            raise Exception
    except Exception:
        print "You need to set DSS_URL, ACCESS_KEY and SECRET_KEY to make a DSS request"
        sys.exit()
...
def gets_dss_signature():
    ...
    auth = ("%s %s:%s" % ("JCS", COMMON_ACCESS_KEY, b64_hmac))
    print("===== Cannonical string =====\n" + cstr + "\n===== END =====\n")
    print("\nAuth Header: " + auth + "\n")
    return auth
