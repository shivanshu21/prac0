...
int RGWPostObj_ObjStore_S3::get_policy()
{
  ...
  if (part_bl("policy", &encoded_policy)) {
    ...
    if (ret < 0) {
      ...
      ACLOwner iam_tenant = getsIamTenant(s); //<<<<<<
      keystone_result = keystone_validator.validate_s3token(s3_access_key,string(encoded_policy.c_str(),encoded_policy.length()),received_signature_str, iam_tenant);
...
int RGW_Auth_S3_Keystone_ValidateToken::validate_s3token(const string& auth_id,
                                                         ...
                                                         ACLOwner& owner) {
  //<<<<<<
  dout(0) << "=================SHIV: Name ksv: " << owner.get_display_name() << dendl;
  dout(0) << "=================SHIV: ID ksv: " << owner.get_id() << dendl;
...
int RGW_Auth_S3::authorize(RGWRados *store, struct req_state *s)
{
  ...
  if (store->ctx()->_conf->rgw_s3_auth_use_keystone
    ...
    if (!rgw_create_s3_canonical_header(s->info, &s->header_time, token, qsr)) {
      ...
      ACLOwner iam_tenant = getsIamTenant(s); //<<<<<<
      keystone_result = keystone_validator.validate_s3token(auth_id, token, auth_sign, iam_tenant);
      if (keystone_result == 0) {
...
/*
 * Returns Tenant for Keystone validation 
 */
//<<<<<<
ACLOwner& getsIamTenant(struct req_state *s)
{
    ACLOwner a;
    RGWAccessControlPolicy* pol = new RGWAccessControlPolicy(s->cct);
    if (pol == NULL) {
        dout(0) << "=================SHIV: Didnt get ACL" << dendl;
    } else {
        return pol->get_owner();
    }
    return a;
}
